...
namespace Parfait {
  namespace Gradient {
    ...
    template <typename T, typename GetEdgeVariable>
    Parfait::Point<T> calculateNodeGradientsFromEdges(const GetEdgeVariable& getEdgeVariable, int num_edges, std::vector<std::array<double, 3>>& ls_weights, const T& center_value){

        Parfait::Point<T> grad(0.0,0.0,0.0);
        for(int i = 0; i < num_edges; i++){
            auto dq = getEdgeVariable(i) - center_value;
            auto c = ls_weights[i];
            grad[0] += c[0] * dq;
            grad[1] += c[1] * dq;
            grad[2] += c[2] * dq;
