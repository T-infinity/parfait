...
class Mesh{
    public:
		...
		Cell<MeshType> cell(int cellId) const
		{return Cell<MeshType>(mesh,cellId);}
    
		int numberOfNodesInCell(int cellId) const
		{return mesh.numberOfNodesInCell(cellId);}
    
		void getNodesInCell(int cellId,int *nodes)
		{mesh.getNodesInCell(cellId,nodes);}
    
		std::vector<int> getNodesInCell(int cellId)
		{return mesh.getNodesInCell(cellId);}
