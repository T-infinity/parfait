...
std::vector<GSWeights> calcGSWeights(GetEdge getEdge, int number_of_edges, GetXyz getXyz, int number_of_nodes){
...
template <typename GetEdge, typename GetXyz, typename Block>
std::vector<std::vector<Parfait::Point<double>>> calculateNodeGradientsFromEdges(
        GetEdge getEdge, int number_of_edges, GetXyz getXyz, 
        int number_of_nodes, const std::vector<Block>& var){

    int blocksize = var[0].size();
    std::vector<std::vector<Parfait::Point<double>>> grad(number_of_nodes);
    for(int i = 0; i < number_of_nodes; i++){
        grad[i] = std::vector<Parfait::Point<double>>(blocksize, {0,0,0});
    }

    auto gs = calcGSWeights(getEdge, number_of_edges, getXyz, number_of_nodes);
    ...
    for(int i = 0; i < number_of_edges; i++){
        ...
        Parfait::Point<double> p{a2[0], a2[1], a2[2]};
        auto dx = p - o;
        std::vector<double> dq(blocksize);
        for(int j = 0; j < blocksize; j++){
            dq[j] = var[node2][j] - var[node1][j];
        }

        auto c = calcLSCoeffs(gs[node1], dx);
        for(int j = 0; j < blocksize; j++){
            grad[node1][j][0] += c[0]*dq[j];
            grad[node1][j][1] += c[1]*dq[j];
            grad[node1][j][2] += c[2]*dq[j];
        }

        dx *= -1;
        c = calcLSCoeffs(gs[node2], dx);
        for(int j = 0; j < blocksize; j++){
            grad[node2][j][0] -= c[0]*dq[j];
            grad[node2][j][1] -= c[1]*dq[j];
            grad[node2][j][2] -= c[2]*dq[j];
        }
    }
    return grad;
}
...
std::vector<Parfait::Point<double>> calculateNodeGradientsFromEdges(
        GetEdge getEdge, int number_of_edges, GetXyz getXyz, 
        int number_of_nodes, const std::vector<double>& var){
