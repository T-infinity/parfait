...
inline std::vector<int> findNodeOwnerRank(std::vector<long>& have, std::set<long>& need, const std::map<long, int>& global_to_local){
    std::vector<int> node_owner(global_to_local.size());
    for(auto global_node_id : have){
        auto local = global_to_local.at(global_node_id);
        node_owner[local] = MessagePasser::Rank();
    }
    ...
    for(int r = 0; r < MessagePasser::NumberOfProcesses(); r++){
        ...
        MessagePasser::Broadcast(transmit, r);
...
inline std::vector<int> setNodeOwnerRank(const std::vector<int>& ownership_degree, const std::vector<long>& globalNodeIds){
    ...
    return findNodeOwnerRank(have, need, global_to_local);
}
...
inline void ParallelMeshReader::createNodeOwnerRank() {
    ...
    meshBuilder->data->nodeOwnerRank = findNodeOwnerRank(have, need, global_to_local);
}
...
inline Parfait::ParallelMeshReader::ParallelMeshReader(std::vector<std::string> gridFiles_in,
        ...
        : isBigEndian(isBigEndian_in), 
          ...
          meshBuilder(std::make_shared<ParallelMeshBuilder>())
{
...
inline void ParallelMeshReader::gatherGhostXyz() {
    ...
    for(int i=0; i < number_of_nodes; ++i)
        if(meshBuilder->data->nodeOwnerRank[i] == MessagePasser::Rank())
            nregular++;
    ...
    for(int proc=0;proc<MessagePasser::NumberOfProcesses();++proc){
        ...
        for(long id:requestedIds){
            if(globalToLocalId.count(id) == 1){
                ...
                if(meshBuilder->data->nodeOwnerRank[localId]==MessagePasser::Rank()) {
