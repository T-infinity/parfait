...
namespace Parfait {
  ...
  inline std::vector<long> NodeBasedRedistributor::redistributeNodeIds() {
      ...
      for (int proc:range(mp->NumberOfProcesses())) {
          int count = 0;
          for (int owner:part)
              if (owner == proc)
                  count++;
  ...
  inline bool
  NodeBasedRedistributor::iShouldSendThisCell(int* cell, int cellSize, const std::vector<long>& neededNodeIds) {
  ...
  inline bool
  NodeBasedRedistributor::iShouldSendThisSurfaceCell(int* cell, int cellSize, const std::vector<long>& neededNodeIds) {
      if (mesh->data->nodeOwnerRank[cell[0]] != mp->Rank()) return false;
      for (int i = 0; i < cellSize; i++) {
          int localNodeId = cell[i];
          long globalNodeId = mesh->data->globalNodeIds[localNodeId];
